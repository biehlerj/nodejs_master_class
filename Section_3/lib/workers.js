      ...
      checks.forEach(function (check) {
        ...
        _data.read('checks', check, function (err, originalCheckData) {
          if (!err && originalCheckData) {
            // Pass the data to a check validator and let that function continue or log errors as needed
...
workers.validateCheckData = function (originalCheckData) {
  ...
  // Set the keys that may not be set if the workers (if the workers have never seen this check before)
  originalCheckData.state = originalCheckData.state = typeof (originalCheckData.state) === 'string' && ['up', 'down'].indexOf(originalCheckData.protocol) > -1 ? originalCheckData.state : 'down';
...
workers.performCheck =  function (originalCheckData) {
  // Prepare the initial the check outcome
  var checkOutcome = {
    'error': false,
    'responseCode': false
  };

  // Mark that the outcome hasn't been set yet
